 ...
 * ({@link java.util.prefs}). Discards values below the minimum or that would
 * leave the window off every screen (e.g. after unplugging a monitor).
 */
public final class WindowState {
    ...
    /** Applies the saved state, if present and valid. Call before {@code show()}. */
    ...
    public static void watch(Stage window) {
        ...
        window.maximizedProperty().addListener((o, a, b) -> PREFS.putBoolean("win.max", b));
    }

    private static void saveIfNotMaximized(Stage window) {
        if (window.isMaximized() || window.isIconified()) {
            return; // keep the "normal" size, not the maximized/minimized one
        }
