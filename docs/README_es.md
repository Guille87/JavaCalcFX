...
herramienta valida los datos de entrada, ejecuta el cálculo fuera del hilo de la
interfaz para que la ventana nunca se congele y muestra el resultado (o un
mensaje de error legible) en la misma pantalla.
...
- **Formato numérico legible**: los decimales se muestran con `#,##0.####` en
  lugar de la representación cruda de `double`.
- **Idioma**: inglés o español, seleccionable desde el menú; la elección se
...
Menú de calculadoras ⇄ pantalla de formulario (instrucciones · campos ·
«Calcular» + resultado · «Volver»). La raíz de la escena es un único contenedor
que siempre muestra **una** pantalla; `Navigator.show(...)` la intercambia.
...
Los métodos de `CalculatorTest` viven en clases `@Nested` (`RightTriangle`,
`CylinderArea`, `LeapYear`, `Factorial`, `Multiples`, `Grades`, …), así que para
apuntar a uno hay que nombrar la clase que lo contiene:
...
Los tests de interfaz (`UiTest`, con TestFX) corren **sin pantalla** mediante
Monocle; no hace falta configurar nada. Para verlos con ventana:
...
`target/site/jacoco/index.html`. La CI publica el HTML como artefacto, comenta la
cobertura en cada *pull request* y actualiza el badge de arriba. Los tests se
ejecutan en la CI con **JDK 17 y 21**.
...
- **`calc/`** — toda la aritmética como métodos `static`, **sin ninguna
  ...
  `CalculationError` con la *clave* del mensaje y sus argumentos; la interfaz
  (`ui/ErrorMessages`) es quien lo traduce. Devuelve `record`s inmutables
  (`Triangle`, `QuadraticEquation`…). `Conversions` pasa libras/pies/pulgadas/
  centímetros a las unidades del SI para reutilizar la misma lógica (lo usa el
  IMC). Es la capa cubierta por tests unitarios.
- **`i18n/`** — `Messages` lee los ficheros `messages*.properties` (inglés de
  base, español encima) y resuelve claves con sustitución de parámetros vía
  `MessageFormat`; `Language` es el enum del selector. El idioma se elige desde
  el menú y se recuerda entre sesiones (`java.util.prefs`).
- **`ui/`** — piezas pequeñas con una sola responsabilidad:
  - `Navigator` — contenedor raíz (`StackPane` con un único hijo); `show(Node)`
    intercambia la pantalla y antes ejecuta un *hook* (cancelar el cálculo en
    curso).
  - `AsyncCalculations` — dueño del `ExecutorService` de un hilo demonio y de la
    `Task` actual. `run(...)` lanza el trabajo fuera del hilo de JavaFX,
    `cancel()` lo interrumpe y `close()` (desde `Application.stop()`) apaga el
    ejecutor. Solo hay un cálculo a la vez.
  - `FormBuilder` — arma la pantalla de formulario genérica (dentro de un
    `ScrollPane`, con el foco en el primer campo). Tras un cálculo correcto
    ofrece un botón «Copiar» (resultado al portapapeles), registra el cálculo en
    el `History` y, si la pantalla lo aporta, «Mostrar pasos». `showWithModes` es
    una variante con un selector que cambia los campos y el cálculo (lo usa el
    IMC para métrico/imperial).
  - `ErrorMessages` — función pura `Throwable → String` (probada con tests).
  - `Format` — formateo numérico puro y seguro entre hilos, con `Locale.ROOT`
    (punto decimal) para no depender del idioma del sistema (probado con tests).
  - `Input` — único punto de parseo de texto a número (probado con tests).
  - `NumericFilter` — `TextFormatter` entero/decimal por campo (probado con
    tests).
  - `WindowState` — persiste tamaño y posición de la ventana (probado con tests).
  - `Theme` — modo claro/oscuro (clase `dark-theme` en `styles.css`), con la
    preferencia persistida en `java.util.prefs` (probado con tests).
  - `History` — los últimos 25 cálculos (título + resultado + fecha), persistidos
    en un subnodo de `java.util.prefs`; el resultado se recorta a 300 caracteres
    para no desbordar el límite de tamaño de las preferencias, y repetir el mismo
    cálculo solo refresca su hora (probado con tests).
  - `LastCalculator` — guarda en `java.util.prefs` la clave de la última
    calculadora abierta; `CalculatorApp` la reabre al arrancar y la olvida al
    volver al menú (probado con tests).
  - `XSteps` — desarrollo paso a paso, en notación lineal, de la cuadrática,
    Pitágoras, el cilindro, el porcentaje y la regla de tres (plantillas
    deterministas, probadas con tests).
  - `Buttons` — fábrica de botones.
- **`CalculatorApp.java`** — `Application` mínima: conecta las piezas de `ui`,
  carga `styles.css` y los iconos, y define un `xScreen()` + entrada de menú por
  calculadora (cada uno declara título, campos, tipo y función de presentación).

`Calculator.factorial` consulta `Thread.isInterrupted()` para abortar pronto un
cómputo largo ya cancelado.

Los identificadores y comentarios del código están en **inglés**; los textos
visibles salen de `i18n` (inglés de base, español incluido).
...
├── README.md · CONTRIBUTING.md · CHANGELOG.md · ROADMAP.md   inglés (principal)
├── CODE_OF_CONDUCT.md · SECURITY.md   ficheros de comunidad
├── docs/                       *_es.md (docs en español) y capturas del README
├── .github/                    workflows de CI y release, badges, Dependabot y plantillas de issues
└── src/
    ├── main/
    │   ├── java/
    │   │   ├── module-info.java              requires javafx.controls y java.prefs; exporta los 4 paquetes
    │   │   └── io/guillermoamadodiaz/javacalcfx/
    │   │       ├── CalculatorApp.java           Application + catálogo de pantallas
    │   │       ├── calc/Calculator.java         lógica matemática pura y validada
    │   │       ├── i18n/                        Messages (lector de .properties) y Language
    ...
    └── test/java/io/guillermoamadodiaz/javacalcfx/
        ├── UiTest.java                         TestFX: navegación y mensajes en pantalla
        ├── calc/CalculatorTest.java            JUnit 5, casos parametrizados y @Nested
...
catetos grandes) y `Math.atan2` para los ángulos (evita `NaN` cuando un cateto es
0). Además de la hipotenusa devuelve área, perímetro y los dos ángulos agudos.
...
distinto de `0` es múltiplo de `0`; por eso `isMultiple(a, 0)` es `true` solo si
...
1. Añade un método puro a `Calculator` con sus precondiciones (lanza
   `CalculationError` con una clave ante una entrada inválida) **y un test**.
2. Añade sus textos (título, instrucciones, campos, resultado,
   `menu.button.<clave>` + `.tooltip` y las claves de error) a
   `messages.properties` y `messages_es.properties`.
3. Crea un método `xScreen()` que llame a `forms.show(...)` con el título, las
   instrucciones, los `prompts`, el `Type` de campo y la función de presentación
   (todo vía `Messages.get(...)`); opcionalmente, una función `steps`.
4. Añade una `MenuEntry("<clave>", this::xScreen)` en la `Category` que
   corresponda de `catalog()`. Si aporta, añade un `ui/XSteps` puro y pásalo como
   argumento `steps`.
...
cómo publicar una versión) están en [`CONTRIBUTING_es.md`](CONTRIBUTING_es.md);
los cambios de cada versión, en [`CHANGELOG_es.md`](CHANGELOG_es.md).
