...
Patrón por cada una: método puro en `Calculator` + test + textos `es`/`en`
(incluido `menu.boton.<clave>` y `.tooltip`) + `xScreen()` + `menuButton`.
...
- [x] Desacoplar `calc` de `Messages`: `CalculationError` lleva la clave del mensaje
      y sus argumentos; `ui/ErrorMessages` traduce. El paquete `calc` ya no
...
- [x] Historial de cálculos (`ui/History`, persistido; pantalla accesible
